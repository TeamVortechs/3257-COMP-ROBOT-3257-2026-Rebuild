...
public class PositiveSlewRateLimiter extends DriveInputFilter{
    ...
    protected ChassisSpeeds runFilter(ChassisSpeeds speeds) {
        return new ChassisSpeeds(xLimiter.calculate(speeds.vxMetersPerSecond), yLimiter.calculate(speeds.vyMetersPerSecond), omegaLimiter.calculate(speeds.omegaRadiansPerSecond));
    }
